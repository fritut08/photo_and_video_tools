"""Host launcher for the Dockerized cpy_geotag_from_xmp_to_photo_files container."""
...
DEFAULT_IMAGE = "cpy-geotag-from-xmp:latest"
CONTAINER_WORKDIR = "/work"

SOURCE_FILES = [
    Path(__file__).parent / "Dockerfile",
    Path(__file__).parent / "container" / "cpy_geotag_from_xmp_to_photo_files.py",
...
def main() -> int:
    print("=" * 60)
    print("  Copy Geotags from XMP to Photo Files")
    print("=" * 60)
    print()
    print("Opening directory selection dialog...")
    directory = select_directory_gui("Select the folder containing XMP + JPG pairs")
